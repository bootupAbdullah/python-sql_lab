    ...
    if results:
        ...
        while True:
            try:
                ...
                if any(row[1] != company_id for row in results):
...
def confirmAddCompany(company):
    print(f"\nCompany: {company.name} Company ID: {company.company_id}")
    confirm_company = input("\nConfirm new Company? (y/n): ")

    if confirm_company.lower() == 'y':
        query = "INSERT INTO companies ( name, company_id) VALUES (%s, %s);"
        try:
            cursor.execute(query, (company.name, company.employer_id))
    ...
    elif confirm_company.lower() =='n':
        print("\nCanceled action")
        add_company_to_db() #callbacks the add employee function if canceled
    
    else:
        print("\nInvalid selection. Please enter 'y' or 'n'.")
        confirmAddCompany(company) #retrys confirmation
